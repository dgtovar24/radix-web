...
export default function AnalyticsPage() {
  ...
  const handlePointClick = (point: any) => {
    ...
    if (pid && (selTable === 'health_metrics' || selTable === 'radiation_logs' || selTable === 'patients')) {
      fetch(`${API}/api/analytics/patient/${pid}/full`).then(r => r.json()).then(setModal);
    }
  };

  const col = columns.find(c => c.key === selX);
  const isNumericY = columns.find(c => c.key === selY)?.type === 'number';
    ...
    <div style={{ display: 'flex', gap: 20, fontFamily: "'Inter', sans-serif", minHeight: 'calc(100vh - 200px)' }}>
      ...
      <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ padding: 16, background: 'var(--sf)', borderRadius: 14, border: '1px solid var(--br)' }}>
          <h3 style={{ fontSize: 12, fontWeight: 800, color: 'var(--t-s)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>Datos</h3>

          <label style={lbl}>Tabla</label>
          <select value={selTable} onChange={e => { setSelTable(e.target.value); setSelX(''); setSelY(''); }} style={sel}>
        ...
        <div style={{ padding: 16, background: 'var(--sf)', borderRadius: 14, border: '1px solid var(--br)' }}>
          <h3 style={{ fontSize: 12, fontWeight: 800, color: 'var(--t-s)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Tipo de Gráfico</h3>
            ...
            {chartTypes.map(ct => {
              ...
              return (
                <button key={ct.id} onClick={() => setSelChart(ct.id)} style={{
                  ...
                  padding: '10px 6px', borderRadius: 10, border: active ? '2px solid var(--p)' : '1px solid var(--br)',
                  background: active ? 'color-mix(in srgb, var(--p) 8%, #fff)' : 'var(--sf)', cursor: 'pointer',
                }}>
                  <Icon size={18} style={{ color: active ? 'var(--p)' : 'var(--t-s)' }} />
                  <span style={{ fontSize: 10, fontWeight: 700, color: active ? 'var(--p)' : 'var(--t-s)' }}>{ct.label}</span>
                </button>
              );
            })}
          </div>
        ...
        {chartData.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--t-s)', gap: 12 }}>
            ...
            <div style={{ fontSize: 11, maxWidth: 300, textAlign: 'center', lineHeight: 1.5 }}>
              Elige una tabla de datos, las columnas para los ejes X e Y, el tipo de gráfico y haz clic en Generar.
              Puedes hacer clic en los puntos del gráfico para ver detalles del paciente.
            </div>
          </div>
        ) : ChartComponents ? (
          <ChartComponents.ResponsiveContainer width="100%" height={400}>
            {selChart === 'bar' ? (
              <ChartComponents.BarChart data={chartData} onClick={(e: any) => e?.activePayload?.[0] && handlePointClick(e.activePayload[0].payload)}>
                ...
                <ChartComponents.Bar dataKey="y" fill="var(--p)" radius={[4,4,0,0]} />
              ...
              <ChartComponents.LineChart data={chartData} onClick={(e: any) => e?.activePayload?.[0] && handlePointClick(e.activePayload[0].payload)}>
                <ChartComponents.CartesianGrid strokeDasharray="3 3" stroke="var(--br)" />
                <ChartComponents.XAxis dataKey="x" tick={{ fontSize: 11, fill: 'var(--t-s)' }} interval={chartData.length > 20 ? Math.floor(chartData.length / 8) : 0} />
                <ChartComponents.YAxis domain={isNumericY ? ['auto', 'auto'] : undefined} tick={{ fontSize: 11, fill: 'var(--t-s)' }} />
                <ChartComponents.Tooltip />
                <ChartComponents.Line type="monotone" dataKey="y" stroke="var(--p)" strokeWidth={3} dot={chartData.length <= 30} />
              ...
              <ChartComponents.AreaChart data={chartData}>
                <ChartComponents.CartesianGrid strokeDasharray="3 3" stroke="var(--br)" />
                <ChartComponents.XAxis dataKey="x" tick={{ fontSize: 11, fill: 'var(--t-s)' }} />
                <ChartComponents.YAxis tick={{ fontSize: 11, fill: 'var(--t-s)' }} />
                <ChartComponents.Tooltip />
                <ChartComponents.Area type="monotone" dataKey="y" stroke="var(--p)" fill="var(--p)" fillOpacity={0.2} />
              </ChartComponents.AreaChart>
            ) : selChart === 'scatter' ? (
              <ChartComponents.ScatterChart>
                <ChartComponents.CartesianGrid strokeDasharray="3 3" stroke="var(--br)" />
                <ChartComponents.XAxis dataKey="x" tick={{ fontSize: 11, fill: 'var(--t-s)' }} />
                <ChartComponents.YAxis dataKey="y" tick={{ fontSize: 11, fill: 'var(--t-s)' }} />
      ...
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--sf)', borderRadius: 18, padding: 28, maxWidth: 600, width: '90%', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
